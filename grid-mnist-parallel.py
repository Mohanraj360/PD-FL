...
def add_rips(d,idx):
    if idx % 10 == 0:
        print(idx)
    rips = Rips(maxdim=2)
    return (rips.fit_transform(d[0], distance_matrix=True),d[1])
...
if __name__ == "__main__":
    ...
    data = []
    for metric in dir:
        if metric[len("vr_metric-mnist-more_layers="):len("vr_metric-mnist-more_layers=")+len("merge_mnist_moreFC_")] == "merge_mnist_moreFC_":
    ...
    for top in range(0,len(data)-79,80):
        print(top)
        with multiprocessing.Pool(4) as pool:
            for idx, d in enumerate(data[top:min(top+80,len(data))]):
                pool.apply_async(
                        add_rips, (d,idx,), callback=get_rips)
