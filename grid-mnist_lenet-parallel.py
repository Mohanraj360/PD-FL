...
if __name__ == "__main__":
    _time = "08-26--15-44-27"
    dir = os.listdir(f"./metric_fed_eval{_time}")
    metric_path = f"./metric_fed_eval{_time}/"
    
    if not os.path.exists(f'./grids_fed_eval{_time}/'):
        os.makedirs(f'./grids_fed_eval{_time}/', exist_ok=True)
    savepth = f"./grids_fed_eval{_time}/"
    
    data = []
    for metric in dir:
#         if metric[len("vr_metric-merge-VGG16_CIFAR="):len("vr_metric-merge-VGG16_CIFAR=")+len("merge_vgg")] == "merge_vgg":
        if "iter" in metric and int(re.findall("\d+\.?\d*", metric)[0]) in range(10,20):
            data.append([np.loadtxt(metric_path+metric), metric])
            # print(metric)
        # if len(data) > 80:
        #     break
    print(len(data))
    ...
    for top in range(0,len(data)-1,80):
        ...
        print(f"progress:{top/len(data)*100}%, eta:{elapsed *(len(data)/(top or 1)-1)} sec")
        print(top)
        ...
        for h1 in H1_dgm:
            ...
            np.savetxt(savepth+"h_"+h1[1], makegrid(h1[0],gridsize(h1[0])))
            print(savepth+"h_"+h1[1])
